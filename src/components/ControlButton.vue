...
<template>
  <div class="relative flex items-center justify-center p-2 touch-none select-none">
    ...
    <button
      ...
      class="relative z-10 flex items-center justify-center shadow-lg transition-shadow active:shadow-sm"
      :class="[
        color,
        shape === 'circle' || !shape ? 'w-16 h-16 rounded-full' : '',
        shape === 'heart' ? 'w-20 h-20 bg-transparent' : '',
        shape === 'heart-left' || shape === 'heart-right' ? 'w-12 h-20 bg-transparent' : ''
      ]"
    >
      <!-- Heart Shape SVG -->
      <svg v-if="shape === 'heart'" viewBox="0 0 100 100" class="w-full h-full drop-shadow-md">
